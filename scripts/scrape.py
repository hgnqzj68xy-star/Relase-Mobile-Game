...
DATA_FILE       = Path(**file**).parent.parent / “data” / “games.json”
BACKUP_FILE     = Path(**file**).parent.parent / “data” / “games.backup.json”
LOOKBACK_DAYS   = 60
LOOKAHEAD_DAYS  = 90
ANDROID_WORKERS = 5
...
try:
v = float(price_val)
return “Free” if v == 0 else f”{v:.2f}EUR”
...
for term in IOS_SEARCH_TERMS:
    try:
        ...
        for item in results:
            ...
            # Deduplication : garder le plus recent
...
log.info(f"iOS total (deduplique) : {len(games)} jeux")
return games
```

# Android via bundleId

def scrape_gplay_page(bundle_id):
...
status = "upcoming" if any(
    ...
    ["pre-register", "preregister", "pre_register", "preregistration", "preinscription"]
) else "released"
...
def fetch_android_from_ios(ios_games, existing_android_ids):
...
log.info(f"  {len(to_fetch)} jeux a verifier ({len(ios_games) - len(to_fetch)} en cache)")
...
with ThreadPoolExecutor(max_workers=ANDROID_WORKERS) as executor:
    ...
    for future in as_completed(futures):
        ...
        # ── CORRECTION CLÉ ──
        # On utilise la date iOS comme référence (fiable)
        # et on enrichit avec les données Google Play (icon, prix, etc.)
        android = {
            "id":          android_id,
            "title":       ios_game["title"],
            "platform":    ["android"],
            "releaseDate": ios_game["releaseDate"],   # date iOS = référence
        ...
        # Fallback icon/header depuis iOS si Google Play n'en a pas
        if not android["icon"]:
        ...
        log.info(f"    -> {status_label} ({android['releaseDate']}) {android['price']}")
        android_games.append(android)
        time.sleep(0.5)

log.info(f"Android total : {len(android_games)} jeux trouves")
return android_games
```

# Merge

def merge_games(existing, *new_lists):
...
# Fusion iOS + Android par titre normalise
...
for group in by_title.values():
    ...
    for sec in group[1:]:
        ...
        # Stocker les deux storeUrls separement
